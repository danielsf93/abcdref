...
/**
 * @file plugins/importexport/abcdref/AbcdrefInfoSender.php
 *
 * Copyright (c) 2013-2020 Simon Fraser University
 * Copyright (c) 2003-2020 John Willinsky
 * Distributed under the GNU GPL v3. For full terms see the file docs/COPYING.
 *
 * @class AbcdrefInfoSender
 * @ingroup plugins_importexport_abcdref
 *
 * @brief Scheduled task to send deposits to Abcdref and update statuses.
 */

import('lib.pkp.classes.scheduledTask.ScheduledTask');


class AbcdrefInfoSender extends ScheduledTask {
	/** @var $_plugin AbcdRefExportPlugin */
	var $_plugin;
	...
	function __construct($args) {
		...
		$plugin = PluginRegistry::getPlugin('importexport', 'AbcdRefExportPlugin'); /* @var $plugin AbcdRefExportPlugin */
		$this->_plugin = $plugin;

		if (is_a($plugin, 'AbcdRefExportPlugin')) {
	 ...
	 */
	function getName() {
		return __('plugins.importexport.abcdref.senderTask.name');
	}
		...
		foreach ($journals as $journal) {
			...
			if ($doiPubIdPlugin->getSetting($journal->getId(), 'enablePublicationDoi')) {
				...
				if (count($unregisteredArticles)) {
					$this->_registerObjects($unregisteredArticles, 'article=>abcdref-xml', $journal, 'articles');
				}
			}
		}
		return true;
	 ...
	 * their articles or issues DOIs sent to Abcdref.
	 * @return array
	 */
