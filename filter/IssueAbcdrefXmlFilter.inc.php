...
/**
 * @file plugins/importexport/abcdref/filter/IssueAbcdrefXmlFilter.inc.php
 *
 * Copyright (c) 2014-2020 Simon Fraser University
 * Copyright (c) 2000-2020 John Willinsky
 * Distributed under the GNU GPL v3. For full terms see the file docs/COPYING.
 *
 * @class IssueAbcdrefXmlFilter
 * @ingroup plugins_importexport_abcdref
 *
 * @brief Class that converts an Issue to a Abcdref XML document.
 */

import('lib.pkp.plugins.importexport.native.filter.NativeExportFilter');

class IssueAbcdrefXmlFilter extends NativeExportFilter {
	/**
	 * Constructor
	 * @param $filterGroup FilterGroup
	 */
	function __construct($filterGroup) {
		$this->setDisplayName('Abcdref XML issue export');
	...
	function getClassName() {
		return 'plugins.importexport.abcdref.filter.IssueAbcdrefXmlFilter';
	}
	...
	function createJournalMetadataNode($doc) {
		...
		/* Both ISSNs are permitted for AbcdRef, so sending whichever one (or both) */
